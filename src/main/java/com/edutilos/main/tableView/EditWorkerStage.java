 ...
 */
public class EditWorkerStage extends Stage {
    //Properties
    ...
    public EditWorkerStage(ObservableList<Worker> data, int selectedIndex) {
        this.data = data;
    ...
    private void populateFields() {
        Worker selected = data.get(selectedIndex);
    ...
    private void addComponents() {
        root = new GridPane();
        lblTitle = new Label("Edit Selected Worker");
        lblId = new Label("Id:");
    ...
    private void registerEvents() {
        btnOk.setOnAction(handler->{
            try {
                ...
                data.set(selectedIndex, new Worker(id, name, age, wage, active));
                this.close();
            } catch(Exception ex) {
                ex.printStackTrace();
